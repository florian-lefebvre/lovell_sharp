...
const minimumLibvipsVersionLabelled =
  process.env.npm_package_config_libvips || pkg.config.libvips;
const minimumLibvipsVersion = semver.coerce(
...
const yarnLocator = () => {
  try {
    ...
    const npmVersion = semver.coerce(
      pkg.optionalDependencies[`@img/sharp-libvips-${buildPlatformArch()}`],
      {
...
const getBrewPkgConfigPath = () => {
  try {
    const brewPrefix = (
      spawnSync('brew', ['--prefix'], { encoding: 'utf8' }).stdout || ''
    ).trim();
    if (brewPrefix) {
...
const getPkgConfigPath = () => {
  try {
    const pkgConfigPath = (
      spawnSync('pkg-config', ['--variable', 'pc_path', 'pkg-config'], {
        encoding: 'utf8'
      }).stdout || ''
    ).trim();
    if (pkgConfigPath) {
...
const useGlobalLibvips = (logger) => {
  ...
  /* node:coverage ignore next */
  return (
    !!globalVipsVersion && semver.gte(globalVipsVersion, minimumLibvipsVersion)
  );
};
