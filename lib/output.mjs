...
const errJp2Save = () =>
  new Error('JP2 output requires libvips with support for OpenJPEG');
...
function toFile(fileOut, callback) {
  ...
  if (!is.string(fileOut)) {
    ...
    !this.constructor.format.jp2.output.file
  ) {
    err = errJp2Save();
  }
  if (err) {
...
function toFormat(format, options) {
  const actualFormat = formats.get(
    (is.object(format) && is.string(format.id)
      ? format.id
      : format
    ).toLowerCase()
  );
...
function jp2(options) {
  ...
  if (is.object(options)) {
    ...
    if (is.defined(options.tileWidth)) {
      if (
        is.integer(options.tileWidth) &&
        is.inRange(options.tileWidth, 1, 32768)
      ) {
    ...
    if (is.defined(options.tileHeight)) {
      if (
        is.integer(options.tileHeight) &&
        is.inRange(options.tileHeight, 1, 32768)
      ) {
...
function tiff(options) {
  if (is.object(options)) {
    ...
    if (is.defined(options.bitdepth)) {
      if (
        is.integer(options.bitdepth) &&
        is.inArray(options.bitdepth, [1, 2, 4])
      ) {
...
function heif(options) {
  if (is.object(options)) {
    if (
      is.string(options.compression) &&
      is.inArray(options.compression, ['av1', 'hevc'])
    ) {
    ...
    if (is.defined(options.bitdepth)) {
      if (
        is.integer(options.bitdepth) &&
        is.inArray(options.bitdepth, [8, 10, 12])
      ) {
    ...
    if (is.defined(options.tune)) {
      if (
        is.string(options.tune) &&
        is.inArray(options.tune, ['auto', 'iq', 'psnr', 'ssim'])
      ) {
...
function jxl(options) {
  if (is.object(options)) {
    ...
    if (is.defined(options.decodingTier)) {
      if (
        is.integer(options.decodingTier) &&
        is.inRange(options.decodingTier, 0, 4)
      ) {
