  ...
  if (is.object(inputOptions)) {
    ...
    if (is.defined(inputOptions.failOn)) {
      if (
        ...
        is.inArray(inputOptions.failOn, [
          'none',
          'truncated',
          'error',
          'warning'
    ...
    if (is.defined(inputOptions.density)) {
      if (
        is.number(inputOptions.density) &&
        is.inRange(inputOptions.density, 1, 100000)
      ) {
    ...
    if (is.defined(inputOptions.limitInputChannels)) {
      if (is.bool(inputOptions.limitInputChannels)) {
        ...
        is.integer(inputOptions.limitInputChannels) &&
        is.inRange(inputOptions.limitInputChannels, 0, Number.MAX_SAFE_INTEGER)
      ) {
        inputDescriptor.limitInputChannels = inputOptions.limitInputChannels;
      } else {
        throw is.invalidParameterError(
    ...
    if (
      is.object(inputOptions.openSlide) &&
      is.defined(inputOptions.openSlide.level)
    ) {
      if (
      ...
      // Deprecated
      if (
        is.integer(inputOptions.level) &&
        is.inRange(inputOptions.level, 0, 256)
      ) {
    ...
    if (is.object(inputOptions.tiff) && is.defined(inputOptions.tiff.subifd)) {
      ...
      // Deprecated
      if (
        is.integer(inputOptions.subifd) &&
        is.inRange(inputOptions.subifd, -1, 100000)
      ) {
    ...
    if (
      is.object(inputOptions.pdf) &&
      is.defined(inputOptions.pdf.background)
    ) {
      ...
      if (
        ...
        if (is.defined(inputOptions.create.pageHeight)) {
          if (
            ...
            if (
              inputOptions.create.height % inputOptions.create.pageHeight !==
              0
            ) {
              throw new Error(
        ...
        if (is.defined(inputOptions.create.noise)) {
          ...
          if (is.defined(inputOptions.create.noise.sigma)) {
            if (
              ...
              inputDescriptor.createNoiseSigma =
    ...
    if (is.defined(inputOptions.text)) {
      if (is.object(inputOptions.text) && is.string(inputOptions.text.text)) {
        ...
        if (
          is.defined(inputOptions.text.height) &&
          is.defined(inputOptions.text.dpi)
        ) {
        ...
        if (is.defined(inputOptions.text.width)) {
          if (
            is.integer(inputOptions.text.width) &&
            inputOptions.text.width > 0
          ) {
        ...
        if (is.defined(inputOptions.text.height)) {
          if (
            is.integer(inputOptions.text.height) &&
            inputOptions.text.height > 0
          ) {
        ...
        if (is.defined(inputOptions.text.wrap)) {
          if (
            ...
            is.inArray(inputOptions.text.wrap, [
              'word',
              'char',
              'word-char',
              'none'
