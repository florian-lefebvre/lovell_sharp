...
suite('Unflatten', () => {
  test('unflatten white background', async (t) => {
    ...
    await t.assert.doesNotReject(() =>
      fixtures.assertSimilar(
        fixtures.expected('unflatten-white-transparent.png'),
        data,
        { threshold: 0 }
      )
    );
  ...
  test('unflatten transparent image', async (t) => {
    t.plan(1);
    const data = await sharp(fixtures.inputPngTrimSpecificColourIncludeAlpha)
      .unflatten()
      .toBuffer();
    await t.assert.doesNotReject(() =>
      fixtures.assertSimilar(
        fixtures.expected('unflatten-flag-white-transparent.png'),
        data,
        { threshold: 0 }
      )
    );
