...
const collect = (metadata, spec) =>
  Object.fromEntries(
    Object.entries(spec).map(([key, selector]) => [
      key,
...
suite('Image metadata', () => {
  ...
  test('JPEG with EXIF/ICC', async (t) => {
    ...
    t.assert.deepStrictEqual(
      collect(metadata, {
        ...
        exifResolutionType: (value) =>
          typeof exifReader(value.exif).Image.XResolution,
        iccType: (value) => typeof value.icc,
  ...
  test('JPEG with IPTC/XMP', async (t) => {
    ...
    t.assert.deepStrictEqual(
      collect(metadata, {
        ...
        xmpStart: (value) =>
          value.xmp.indexOf(Buffer.from('<?xpacket begin="')),
        xmpStringStarts: (value) =>
          value.xmpAsString.startsWith(
  ...
  test('PNG with greyscale bKGD chunk - 8 bit', async (t) => {
    ...
    const metadata = await sharp(
      fixtures.inputPng8BitGreyBackground
    ).metadata();
    t.assert.deepStrictEqual(
      collect(metadata, {
  ...
  test('PNG with greyscale bKGD chunk - 16 bit', async (t) => {
    t.plan(1);
    const metadata = await sharp(
      fixtures.inputPng16BitGreyBackground
    ).metadata();
    t.assert.deepStrictEqual(
      collect(metadata, {
        background: 'background',
  ...
  test('Animated WebP with all pages', async (t) => {
    t.plan(1);
    const metadata = await sharp(fixtures.inputWebPAnimated, {
      pages: -1
    }).metadata();
    t.assert.deepStrictEqual(
      collect(metadata, {
  ...
  test('keep existing ICC profile', async (t) => {
    ...
    t.assert.strictEqual(
      icc.parse(metadata.icc).description,
      'Generic RGB Profile'
    );
  });
  ...
  test('transform with but discard existing RGB input profile for CMYK output', async (t) => {
    ...
    const metadata = await sharp(data).metadata();
    t.assert.deepStrictEqual(
      { channels: metadata.channels, icc: metadata.icc },
      { channels: 4, icc: undefined }
    );
  });
  ...
  test('transform to invalid ICC profile emits warning', async (t) => {
    ...
    const metadata = await sharp(data).metadata();
    t.assert.deepStrictEqual(
      { warningsEmitted, channels: metadata.channels, icc: metadata.icc },
      {
  ...
  test('Apply custom output ICC profile', async (t) => {
    ...
    t.assert.doesNotThrow(() =>
      fixtures.assertMaxColourDistance(
        output,
        fixtures.expected('hilutite.jpg'),
        9
      )
    );
  });
  ...
  test('Add EXIF metadata to JPEG', async (t) => {
    ...
    t.assert.deepStrictEqual(
      {
        software: parsedExif.Image.Software,
        exposureTime: parsedExif.Photo.ExposureTime
      },
      {
        software: 'sharp',
        exposureTime: 0.2
  ...
  test('16-bit TIFF with TIFFTAG_PHOTOSHOP metadata', async (t) => {
    ...
    t.assert.deepStrictEqual(
      collect(metadata, {
        ...
        tifftagPhotoshopBuffer: (value) =>
          Buffer.isBuffer(value.tifftagPhotoshop),
  ...
  test('File input with corrupt header fails gracefully', async (t) => {
    t.plan(1);
    await t.assert.rejects(
      () => sharp(fixtures.inputJpgWithCorruptHeader).metadata(),
      /Input file has corrupt header: VipsJpeg: premature end of/
    );
  });
  ...
  test('Buffer input with corrupt header fails gracefully', async (t) => {
    ...
    await t.assert.rejects(
      ...
      /Input buffer has corrupt header: VipsJpeg: premature end of/
    );
  });
  ...
  test('keepExif maintains all EXIF metadata', async (t) => {
    ...
    t.assert.deepStrictEqual(
      { copyright: exif2.Image.Copyright, software: exif2.Image.Software },
      {
        copyright: 'Test 1',
        software: 'sharp'
      }
  ...
  test('withExif replaces all EXIF metadata', async (t) => {
    ...
    t.assert.deepStrictEqual(
      {
        firstCopyright: exif1.Image.Copyright,
        firstSoftware: exif1.Image.Software,
        secondCopyright: exif2.Image.Copyright,
        secondSoftware: exif2.Image.Software
      },
      {
  ...
  suite('XMP metadata tests', () => {
    ...
    test('withXmp with custom XMP replaces existing XMP', async (t) => {
      ...
      t.assert.deepStrictEqual(
        collect(metadata, {
          xmpType: (value) => typeof value.xmp,
          xmpBuffer: (value) => Buffer.isBuffer(value.xmp),
          includesCreator: (value) =>
            value.xmp.toString().includes('Test Creator'),
    ...
    test('withXmp with custom XMP buffer on image without existing XMP', async (t) => {
      t.plan(1);
      const customXmp =
        '<?xml version="1.0"?><x:xmpmeta xmlns:x="adobe:ns:meta/"><rdf:RDF xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns#"><rdf:Description rdf:about="" xmlns:dc="http://purl.org/dc/elements/1.1/"><dc:description><rdf:Alt><rdf:li xml:lang="x-default">Added via Sharp</rdf:li></rdf:Alt></dc:description></rdf:Description></rdf:RDF></x:xmpmeta>';
      const data = await sharp(fixtures.inputJpg)
        .resize(320, 240)
        .withXmp(customXmp)
        .toBuffer();
      ...
      t.assert.deepStrictEqual(
        collect(metadata, {
          ...
          xmpBuffer: (value) => Buffer.isBuffer(value.xmp),
          includesText: (value) =>
            value.xmp.toString().includes('Added via Sharp')
        }),
        {
          xmpType: 'object',
          xmpBuffer: true,
    ...
    test('withXmp with valid XMP metadata for different image formats', async (t) => {
      t.plan(1);
      const customXmp =
        '<?xml version="1.0"?><x:xmpmeta xmlns:x="adobe:ns:meta/"><rdf:RDF xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns#"><rdf:Description rdf:about="" xmlns:dc="http://purl.org/dc/elements/1.1/"><dc:subject><rdf:Bag><rdf:li>test</rdf:li><rdf:li>metadata</rdf:li></rdf:Bag></dc:subject></rdf:Description></rdf:RDF></x:xmpmeta>';
    ...
    test('XMP metadata persists through multiple operations', async (t) => {
      t.plan(1);
      const customXmp =
        '<?xml version="1.0"?><x:xmpmeta xmlns:x="adobe:ns:meta/"><rdf:RDF xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns#"><rdf:Description rdf:about="" xmlns:dc="http://purl.org/dc/elements/1.1/"><dc:identifier>persistent-test</dc:identifier></rdf:Description></rdf:RDF></x:xmpmeta>';
      const data = await sharp(fixtures.inputJpg)
        .resize(320, 240)
        .withXmp(customXmp)
        .rotate(90)
      ...
      t.assert.deepStrictEqual(
        collect(metadata, {
          ...
          xmpBuffer: (value) => Buffer.isBuffer(value.xmp),
          includesText: (value) =>
            value.xmp.toString().includes('persistent-test')
        }),
        {
          xmpType: 'object',
          xmpBuffer: true,
    ...
    test('withXmp XMP works with WebP format specifically', async (t) => {
      ...
      t.assert.deepStrictEqual(
        collect(metadata, {
          ...
          xmpBuffer: (value) => Buffer.isBuffer(value.xmp),
          includesCreator: (value) =>
            value.xmp.toString().includes('WebP Creator'),
