...
suite('Image channel extraction', () => {
  test('Red channel', async (t) => {
    ...
    await t.assert.doesNotReject(() =>
      fixtures.assertSimilar(fixtures.expected('extract-red.jpg'), data)
    );
  });
  ...
  test('Green channel', async (t) => {
    ...
    t.assert.strictEqual(240, info.height);
    await t.assert.doesNotReject(() =>
      fixtures.assertSimilar(fixtures.expected('extract-green.jpg'), data)
    );
  });
  ...
  test('Blue channel', async (t) => {
    ...
    t.assert.strictEqual(240, info.height);
    await t.assert.doesNotReject(() =>
      fixtures.assertSimilar(fixtures.expected('extract-blue.jpg'), data)
    );
  });
  ...
  test('Alpha from 16-bit PNG', async (t) => {
    ...
    await t.assert.doesNotThrow(() =>
      fixtures.assertMaxColourDistance(
        output,
        fixtures.expected('extract-alpha-16bit.png')
      )
    );
  });
  ...
  test('Alpha from 2-channel input', async (t) => {
    ...
    await t.assert.doesNotThrow(() =>
      fixtures.assertMaxColourDistance(
        output,
        fixtures.expected('extract-alpha-2-channel.png')
      )
    );
  });
